...
Supports:
- Qwen3-0.6B, 1.7B, 4B, 8B, etc.

Key differences from Llama:
- QK-norm (RMSNorm on queries and keys before attention)
...
class ModelArgs(BaseModelArgs):
    """Qwen3 model configuration arguments."""
...
class Attention(nn.Module):
    """Multi-head attention with QK-norm (Qwen3 specific)."""
    ...
    def __init__(self, args: ModelArgs):
        ...
        # Qwen3 uses no bias on projections
        self.q_proj = nn.Linear(dim, n_heads * head_dim, bias=False)
        self.k_proj = nn.Linear(dim, n_kv_heads * head_dim, bias=False)
        self.v_proj = nn.Linear(dim, n_kv_heads * head_dim, bias=False)
        self.o_proj = nn.Linear(n_heads * head_dim, dim, bias=False)

        # QK-norm: normalize queries and keys
...
class Qwen3Model(nn.Module):
    """Qwen3 transformer backbone (embeddings + layers + norm)."""
