"""Qwen2/Qwen2.5 model architecture for LMForge.

Supports:
- Qwen2.5-0.5B, 1.5B, 3B, 7B, 14B, 32B, 72B
- DeepSeek-R1-Distill-Qwen (1.5B, 7B) — uses Qwen2 architecture

Key differences from Qwen3:
- No QK-norm (standard attention without query/key normalization)
- Optional bias on projections (usually False for Qwen2.5)
- Same SwiGLU activation and RoPE as Qwen3
...
class ModelArgs(BaseModelArgs):
    """Qwen2/Qwen2.5 model configuration arguments."""
...
class Attention(nn.Module):
    """Multi-head attention (standard, no QK-norm)."""
...
class Qwen2Model(nn.Module):
    """Qwen2 transformer backbone (embeddings + layers + norm)."""
...
class Model(nn.Module):
    """Qwen2/Qwen2.5 model for LMForge training."""
