"""FastAPI application for CortexLab Studio.
...
def create_app(runs_dir: str = "~/.cortexlab/runs") -> FastAPI:
    ...
    async def ws_inference(websocket: WebSocket):
        ...
        try:
            while True:
                ...
                try:
                    from cortexlab import generate as cortexlab_generate

                    token_gen = cortexlab_generate(
                        model=model_path,
                        prompt=prompt,
                        messages=messages,
...
def run_server(host: str = "127.0.0.1", port: int = 8741, runs_dir: str = "~/.cortexlab/runs"):
