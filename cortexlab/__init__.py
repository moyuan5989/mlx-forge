"""CortexLab — LoRA SFT training framework for MLX on Apple Silicon."""
...
def prepare(
    ...
    *,
    ...
    Args:
        ...
        output: Ignored (kept for CLI compat). Storage is now in ~/.cortexlab/datasets/
    ...
    from cortexlab.models.resolve import resolve_model
...
def train(config, resume: str | None = None):  # -> TrainState
    ...
    from cortexlab.data import backend
    from cortexlab.manifest import write_manifest
    from cortexlab.models.loader import load_model
    from cortexlab.models.resolve import resolve_model
    from cortexlab.trainer.callbacks import ConsoleCallback, MetricsLoggerCallback
    from cortexlab.trainer.trainer import Trainer
    ...
    if hasattr(config.training, 'wandb_project') and config.training.wandb_project:
        try:
            from cortexlab.trainer.callbacks import WandBCallback
            callbacks.append(
                WandBCallback(
...
def _validate_resume(resume_path: Path, config) -> None:
    ...
    if state.get("schema_version", 1) > 1:
        raise ValueError(
            ...
            f"supported version 1. Please upgrade CortexLab."
        )
