"""SimPO Trainer for MLX Forge.

Simple Preference Optimization — length-normalized log prob as implicit reward.
Same data format as DPO (paired preferences).
"""
...
class SimPOTrainer(BaseTrainer):
    """SimPO trainer — simple preference optimization with length normalization."""
    ...
    def __init__(self, model, config, train_dataset, val_dataset,
        ...
        self.beta = config.training.simpo_beta
        self.gamma = config.training.simpo_gamma
        self._sft_loss = SFTLoss()

    def _build_step_functions(self, compile_state, apply_grad_update):
        """Build compiled SimPO step functions."""
        from mlx_forge.losses.preference import simpo_loss

        def _simpo_loss_fn(model, chosen_ids, chosen_labels,
        ...
        loss_value_and_grad = nn.value_and_grad(self.model, _simpo_loss_fn)
    ...
    def _execute_step(self, step_fns, batch_data, grad_accum, do_update, compile_state):
        ...
        loss, toks, grad_accum = step_fns["simpo"](
    ...
    def _evaluate_preference(self) -> float:
        """Evaluate using SimPO loss on preference pairs."""
        from mlx_forge.losses.preference import simpo_loss
