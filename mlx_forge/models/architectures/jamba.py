"""Jamba hybrid Mamba + Attention architecture for MLX Forge.

Supports:
- AI21 Jamba, Jamba-1.5

Key features:
- Alternating Mamba SSM and attention layers
- Some layers have MoE
- Mixed cache types (RecurrentCache for Mamba, KVCache for attention)
...
class ModelArgs(BaseModelArgs):
    """Jamba model configuration arguments."""
    ...
    def __post_init__(self):
        ...
        if self.mamba_intermediate_size is None:
            self.mamba_intermediate_size = self.hidden_size * self.expand
...
class MambaLayer(nn.Module):
    """Mamba SSM layer for Jamba hybrid model."""

    def __init__(self, args: ModelArgs):
        super().__init__()

        self.d_model = args.hidden_size
        self.d_inner = args.mamba_intermediate_size
...
class Attention(nn.Module):
    """Multi-head attention with GQA for Jamba."""
...
class JambaBlock(nn.Module):
    """Jamba block: either Mamba or Attention, with dense or MoE MLP."""
    ...
    def __call__(
        ...
        if self.use_attention:
            ...
            h = self.mamba(h, cache)

        x = x + h
        r = self.mlp(self.pre_ff_layernorm(x))
        return x + r


class JambaModel(nn.Module):
    """Jamba transformer backbone."""
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.layers = [
            JambaBlock(args, layer_idx=i) for i in range(args.num_hidden_layers)
        ]
    ...
    def __call__(self, inputs: mx.array, cache=None):
        ...
        # Build attention mask only for attention layers
...
class Model(nn.Module):
    """Jamba hybrid model for MLX Forge."""
    ...
    def make_cache(self):
        """Create mixed cache types: KVCache for attention, RecurrentCache for Mamba."""
        caches = []
