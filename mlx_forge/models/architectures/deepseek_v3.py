"""DeepSeek V3/R1 architecture for MLX Forge.
...
Key features:
- MoE + MLA (same as V2) with routed_scaling_factor
- Multi-token prediction (MTP) head support
...
class ModelArgs(BaseModelArgs):
    """DeepSeek V3 model configuration arguments."""
...
class MoELayer(nn.Module):
    """DeepSeek V3 MoE layer with routed scaling factor."""
...
class Model(nn.Module):
    """DeepSeek V3/R1 model for MLX Forge."""
    ...
    def sanitize(self, weights: dict) -> dict:
        ...
        # Remove MTP head weights if present (not used during training)
        weights = {
            k: v for k, v in weights.items() if not k.startswith("model.mtp_")
        }
        if self.args.tie_word_embeddings:
